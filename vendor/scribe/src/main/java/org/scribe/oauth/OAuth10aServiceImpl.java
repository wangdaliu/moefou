...
public class OAuth10aServiceImpl implements OAuthService {
    ...
    private void addOAuthParams(OAuthRequest request, String secret) {
    ...
    public void signRequest(Token token, OAuthRequest request) {
        ...
        addOAuthParams(request, token.getSecret());
        appendSignature(request);
    }
    ...
    @Override
    public String getAuthorizationHeader(Verb verb, String secret, String token, String verifier, String path) {
        OAuthRequest request = new OAuthRequest(verb, path);
        request.addOAuthParameter(OAuthConstants.TOKEN, token);
        request.addOAuthParameter(OAuthConstants.VERIFIER, verifier);
        addOAuthParams(request, secret);
        String oauthHeader = api.getHeaderExtractor().extract(request);
        return oauthHeader;
    }

    public String getSignature(OAuthRequest request, String secret) {
        config.log("generating signature...");
        config.log("using base64 encoder: " + Base64Encoder.type());
        String baseString = api.getBaseStringExtractor().extract(request);
        String signature = api.getSignatureService().getSignature(baseString, config.getApiSecret(), secret);
